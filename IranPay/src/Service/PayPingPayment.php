...
use IranPay\Models\PayPing\PayPing;
 ...
 */
class PayPingPayment implements AsynchronousPaymentHandlerInterface
{
    /**
    ...
    public function __construct(
        ...
        $this->token = $setting->getPayPingToken();
    }
    ...
    public function pay(
        ...
        try {
            ...
            $data = [
                'clientRefId' => $transaction->getOrder()->getOrderNumber(),
                'payerIdentity' => $customer->getDefaultBillingAddress()->getPhoneNumber() ?? $customer->getEmail(),
                "payerName" => $customer->getActiveBillingAddress()->getFirstName() . ' ' . $customer->getActiveBillingAddress()->getLastName(),
            ...
            $redirectUrl = (new PayPing($this->token, $transaction))->pay($data);
        } catch (\Throwable $e) {
            $this->insertToDB->storeIranPayTransactionData(
                $transaction,
                $salesChannelContext,
...
//            $this->transactionStateHandler->reopen($transaction->getOrderTransaction()->getId(), $context);
        ...
        try {
            $payping = new PayPing($this->token, $transaction);
            $paymentState = $payping->verify($refID, (integer)$transaction->getOrder()->getAmountTotal());
        ...
        if ($paymentState) {
            ...
            $this->insertToDB->storeIranPayTransactionData(
                ...
                $transaction->getOrder()->getOrderNumber(),
                null,
                'Error in verifying payment line 164'
            );
